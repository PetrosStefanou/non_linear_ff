...
for i in range(0, len(R)-1):
    for j in range(1, len(TH)-1):
        ...
        #use 2nd order lax-wendroff discretisation scheme
...
cont = ax.contour(x, z, alpha, cmap=cm.Blues , levels=10)
#analytical value of alpha
cont_an = ax.contour(x, z, alpha_an, cmap=cm.Reds, levels=10)
...
lines = [ cont.collections[-1], cont_an.collections[-1]]
...
ax.set_xlabel('x')
ax.set_ylabel('z')
ax.set_title(r'$| \alpha_{an} - \alpha_{num} |$')
# ax.set_xlim(0, 1)
# ax.set_ylim(0, 1)
